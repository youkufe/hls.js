...
var AudioStreamController = function (_EventHandler) {
  ...
  function AudioStreamController(hls) {
    ...
    var _this = _possibleConstructorReturn(this, _EventHandler.call(this, hls, Event.MEDIA_ATTACHED, Event.MEDIA_DETACHING, Event.AUDIO_TRACKS_UPDATED, Event.AUDIO_TRACK_SWITCHING, Event.AUDIO_TRACK_LOADED, Event.KEY_LOADED, Event.FRAG_LOADED, Event.FRAG_PARSING_INIT_SEGMENT, Event.FRAG_PARSING_DATA, Event.FRAG_PARSED, Event.ERROR, Event.BUFFER_CREATED, Event.BUFFER_APPENDED, Event.BUFFER_FLUSHED, Event.INIT_PTS_FOUND));
        ...
        if (!appendOnBufferFlush && pendingData.length) {
          pendingData.forEach(function (appendObj) {
            ...
            if (_this2.state === State.PARSING) {
              ...
              _this2.hls.trigger(Event.BUFFER_APPENDING, appendObj);
            }
          });
          this.pendingData = [];
          this.appended = true;
        ...
        if (data.parent === 'audio' && (this.state === State.PARSING || this.state === State.PARSED)) {
          ...
          if (mediaBuffered) {
            ...
            if (_config.maxMaxBufferLength >= _config.maxBufferLength) {
              ...
              logger.warn('audio:reduce max buffer length to ' + _config.maxMaxBufferLength + 's');
              // increase fragment load Index to avoid frag loop loading error after buffer flush
  ...
  AudioStreamController.prototype.onBufferFlushed = function onBufferFlushed() {
    ...
    if (pendingData && pendingData.length) {
      ...
      pendingData.forEach(function (appendObj) {
        _this3.hls.trigger(Event.BUFFER_APPENDING, appendObj);
      });
      this.appended = true;
      this.pendingData = [];
      this.state = State.PARSED;
