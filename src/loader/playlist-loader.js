...
const MASTER_PLAYLIST_TARTGETDURATION_REGEX = /EXT-X-TARGETDURATION:([0-9\.\-]+)/;
    ...
    if (string.indexOf('#EXTM3U') === 0) {
      if (string.indexOf('#EXTINF:') > 0) {
        ...
        if (levels.length) {
          ...
          let result;
          MASTER_PLAYLIST_REGEX.lastIndex = 0;
          const matchs = string.match(MASTER_PLAYLIST_TARTGETDURATION_REGEX);
          const targetDuration = matchs[1] || 'invalid';

          hls.trigger(Event.MANIFEST_LOADED, {levels, targetDuration, string, url,  stats, networkDetails});
          hls.trigger(Event.ERROR, {type: ErrorTypes.NETWORK_ERROR, levels: levels, details: ErrorDetails.MANIFEST_PARSING_ERROR, fatal: true, url: url, reason: 'no level found in manifest', networkDetails: networkDetails});
        }
      }
    } else {
